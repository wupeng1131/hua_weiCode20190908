...
class Map:
    ...
    # def is_overlap(self,_x,_y):
    #     for w in self.wander_task:
    #         if w.x == _x and w.y == _y:
    #             print "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^overlap"
    #             return True
    #     return False
    ...
    def wander_task_check(self,own,mode):
        ...
        count = self.around_count(x1,y1)
        if count>=1:
            _x = x1
            _y = y1
            # print "^^^^^^^^^^^^^^^^^^ cannot reach position is", _x, _y
            while self.around_count(_x,_y)>=1 or not self.is_space(_x,_y) :

                _x, _y = rand_choose_1(self.width, self.height)
            # print "***************choose next:", _x, _y
            _task = task(_x, _y, value)
            _task.info = "wander_task"
            _task.ownIndex = idx
            self.wander_task[idx] = _task
    ...
    def add_own_block(self,own,enemy):
        p = []
        p.append([own.x,own.y])
        p.append([own.x-1, own.y])
        p.append([own.x+1, own.y])
        p.append([own.x, own.y-1])
        p.append([own.x, own.y+1])
        for i in range(5):
            # distance && not me && has_space
            if self.not_me(p[i][0], p[i][1],enemy) and self.has_space(p[i][0], p[i][1]):
                # == 0
                _x = p[i][0]
                _y = p[i][1]
    ...
    def add_enemy(self,enemy, me):
        ...
        for i in range(5):
            # distance && not me && has_space
            if distance_(me.x, me.y, p[i][0], p[i][1]) <=constants.add_enemy_vision and self.not_me(p[i][0], p[i][1],me) and self.has_space(p[i][0], p[i][1]):
                # == 0
                _x = p[i][0]
                _y = p[i][1]
                if self.map[_y][_x] == 0:# or self.map[_y][_x] == 3:
        ...
        for i in range(5):
            # distance && not me && has_space
            if self.is_tunnel(p[i][0], p[i][1]):
                # == 0
                _x = p[i][0]
                _y = p[i][1]
                x2,y2 = self.run_tunnel(_x,_y)
                if self.map[y2][x2] == 0 and self.not_me(x2, y2,me):
                    self.map[y2][x2] = 1
